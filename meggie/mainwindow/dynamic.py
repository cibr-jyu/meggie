                ...
                with open(config_path, "r") as f:
                    ...
                    if config:
                        if config["id"] in found_keys:
                            raise Exception(
                                "Datatype with the same id found "
    ...
    class DynamicTab(QtWidgets.QWidget):
        ...
        def __init__(self, parent):
            ...
            for idx, input_name in enumerate(self.tab_spec["inputs"]):
                ...
                setattr(
                    self, "groupBoxInputElement_" + str(idx + 1), groupBoxInputElement
                )
                setattr(
                    self,
                    "gridLayoutInputElement_" + str(idx + 1),
                    gridLayoutInputElement,
                )
                setattr(
                    self,
                    "listWidgetInputElement_" + str(idx + 1),
            ...
            for idx, output_name in enumerate(self.tab_spec["outputs"]):
                ...
                setattr(
                    self, "groupBoxOutputElement_" + str(idx + 1), groupBoxOutputElement
                )
                setattr(
                    self,
                    "gridLayoutOutputElement_" + str(idx + 1),
                    gridLayoutOutputElement,
                )
                setattr(
                    self,
                    "listWidgetOutputElement_" + str(idx + 1),
            ...
            for idx, action_name in enumerate(self.tab_spec["input_actions"]):
                ...
                setattr(
                    self,
                    "pushButtonInputActionElement_" + str(idx + 1),
            ...
            for idx, action_name in enumerate(self.tab_spec["output_actions"]):
                ...
                setattr(
                    self,
                    "pushButtonOutputActionElement_" + str(idx + 1),
            ...
            for idx, info_name in enumerate(self.tab_spec["info"]):
                ...
                setattr(
                    self, "groupBoxInfoElement_" + str(idx + 1), groupBoxInfoElement
                )
                setattr(
                    self, "gridLayoutInfoElement_" + str(idx + 1), gridLayoutInfoElement
                )
                setattr(
                    self,
                    "plainTextEditInfoElement_" + str(idx + 1),
            ...
            if self.tab_spec["input_actions"] and not self.tab_spec["inputs"]:
                self.gridLayoutRoot.addWidget(self.groupBoxInputActions, 0, 0, 2, 1)
            elif self.tab_spec["input_actions"]:
                self.gridLayoutRoot.addWidget(self.groupBoxInputActions, 1, 0, 1, 1)
            ...
            for idx, info_name in enumerate(self.tab_spec["info"]):
                info_element = getattr(self, "plainTextEditInfoElement_" + str(idx + 1))
        ...
        def _get_data(self):
            ...
            for idx, name in enumerate(self.tab_spec["inputs"]):
                ...
                try:
                    selected_items = [
                        item.text() for item in ui_element.selectedItems()
                    ]
