...
from meggie.utilities.dialogs.powerSpectrumAddAdvancedDialogMain import (
        ...
        for ival_type, interval in intervals:
            ...
            if ival_type == "fixed":
                item = QtWidgets.QListWidgetItem(
                    "%s: %s - %s s (fixed)"
                    % (interval[0], round(interval[1], 4), round(interval[2], 4))
                )
            else:
                item_string = str(interval[0]) + ": ["
                if interval[1][0] == "events":
                    item_string += ", ".join([str(elem) for elem in interval[1][1:]])
                ...
                if interval[2][0] == "events":
                    item_string += ", ".join([str(elem) for elem in interval[2][1:]])
