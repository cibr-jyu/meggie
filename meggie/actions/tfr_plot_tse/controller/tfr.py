...
def plot_tse_topo(
    subject, tfr_name, blmode, blstart, blend, tmin, tmax, fmin, fmax, ch_type
):
...
def plot_tse_averages(
    subject, tfr_name, blmode, blstart, blend, tmin, tmax, fmin, fmax, channel_groups
):
    """Plots tse averages."""
    meggie_tfr = subject.tfr.get(tfr_name)
    ...
    for ch_type in ch_types:
        ...
        def plot_fun(ax_idx, ax):
            ...
            for color_idx, (key, times, curve) in enumerate(
                averages[(ch_type, ch_group)]
            ):
