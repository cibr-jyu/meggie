...
def group_average_spectrum(experiment, spectrum_name, groups, new_name):
    ...
    for group_key, group_subjects in groups.items():
        for subject in experiment.subjects.values():
            ...
            for spectrum_item_key, spectrum_item in spectrum.content.items():
                ...
                idxs = [subject_ch_names.index(ch_name) for ch_name in common_ch_names]
    ...
    common_idxs = [
        ch_idx
        for ch_idx, ch_name in enumerate(clean_names(info["ch_names"]))
    ...
    spectrum = Spectrum(new_name, subject.spectrum_directory, params, data, freqs, info)
